          ...
          {cart.items.map(item => (
            <div key={item.productId} className="card" style={{ display: 'flex', gap: 16, padding: 16, marginBottom: 12 }}>
              <div style={{ width: 100, height: 100, borderRadius: 8, overflow: 'hidden', background: 'var(--gray-50)', flexShrink: 0 }}>
                {item.productImage ? <img src={getImageUrl(item.productImage)} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <div style={{ fontSize: 40, display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%' }}>💻</div>}
              </div>
              <div style={{ flex: 1 }}>
